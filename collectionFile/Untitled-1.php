...
function dueAmtCalculation($con, $start_date, $end_date, $due_amt, $int_rate)
{
    ...
    $qry = $con->query("SELECT princ_amt_track as princ,bal_amt, coll_date FROM `collection` WHERE req_id = '" . $GLOBALS['req_id'] . "' and princ_amt_track != '' and month(coll_date) = month($start_date) and year(coll_date) = year($start_date) ORDER BY coll_date ASC ");
    if ($qry->num_rows > 0) {
...
function payableCalculation($con, $loan_arr, $response)
{
    ...
    while ($st_date->format('m') <= $last_month->format('m')) {
        ...
        $result += dueAmtCalculation($con, $st_date, $end_date, $response['due_amt'], $loan_arr['int_rate']);
